...
_Code review 2026-08-06 — three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) over `0757ac9..c8e496a`. All ACs judged MET by the auditor; the items below are defects and open calls found alongside that verdict._

**Decisions — resolved 2026-08-06 by Alexis**

- [x] [Review][Decision] **Theory-board submit control (Open Question 3) — RATIFIED, with both gaps fixed.** `ColleagueRenderer` grew a `SUBMIT_*` geometry block, a rectangle + label on the conclusion board, `submitConclusion()`, and `PhaserStoreAdapter.submitConclusion`, none of which any task authorized. Ratified: AC1's "when the choice is submitted" and Task 10's e2e are unreachable without it, it is Phaser rather than DOM parity, it is released in `destroy()`, and it respects overlay suppression. Open Question 3 is closed in favour of its first reading — choosing stays freely revisable, submitting is the deliberate act that invites the challenge. Two consequences became patches: the silent defensible-submit path and the guide copy. Amend the Task list to cover the control so the checked boxes match the code.
- [x] [Review][Decision] **Critique line length — bound it in the schema.** Deviation 6 anchored the *revise control* to the canvas floor so the way back always exists, but the *prose* has no clamp: `RivalLabRenderer.layout()` chases `speakerY`/`bodyY` upward with no ceiling, and only the guide is clamped. Resolution: add a `.max()` to the critique `line` in `RivalLabCritiqueSchema` so over-long content fails at case load, where an author can see it — rather than clamping the body, which would silently truncate the objection that Task 7 says must never be truncated. Became a patch.
- [x] [Review][Decision] **Clock-regression refusal — accepted as specified.** `AppState.ts:699-702` requires a timestamp strictly later than the last `critiqueHistory` entry, and `PhaserStoreAdapter.ts:79` stamps it from `new Date()`, so a record with a future last-challenge timestamp refuses every submit until the wall clock catches up. The strictness is exactly the `reduceRevisionSave` discipline Task 4 asked for and no progress is ever lost, so it stands. The actionable part is the misleading message, which the distinct-error-code patch below already fixes. No further change.
- [x] [Review][Decision] **`critiqueHistory` display — deferred to a later story.** AC2 is satisfied by the durable fact: the record is in IndexedDB and in the exported JSON. A reader-facing surface belongs with the debrief/print pass, not here — `DebriefScene` is still a `PhasePlaceholderScene`, so building it now would expand scope well past 2.5. Logged in `deferred-work.md`.

**Patches**

- [x] [Review][Patch] A defensible submit is a silent no-op, and no guide copy mentions submitting [src/core/store/AppState.ts:709-711, src/adapters/phaser/renderers/ColleagueRenderer.ts:291-297, src/core/i18n/locales/en.ts:566, src/core/i18n/locales/fr.ts:603] — from the ratification above. `submitConclusion()` returns early on success and the defensible branch changes nothing any surface reads, so the success path is indistinguishable from a dead button; and neither locale's `theoryBoard.guide` tells the player that submitting is a separate deliberate act, so the rival lab may never be reached in normal play. Add a success signal on the defensible path and update the guide copy in **both** locales.
- [x] [Review][Patch] The critique line has no maximum length [src/schemas/CaseDefinitionSchema.ts, `RivalLabCritiqueSchema`] — from the decision above. Add a `.max()` to `line`'s EN and FR strings so an over-long objection fails at case load rather than overlapping the guide and running off a non-scrolling canvas at runtime.

- [x] [Review][Patch] `rivalLabCritique` survives every phase transition, so a stale challenge pins the router across phases [src/core/store/AppState.ts:614, src/core/store/AppState.ts:772] — the diff adds the clear to eleven evidence-touching reducers and to none of the phase-moving ones. `reduceCasePhaseAdvance:614` and `reduceDebriefComplete:772` both return without it, while `SceneRouter.ts` makes the critique an unconditional, phase-blind override. Reachable: the retired-but-mounted DOM panels are outside canvas input suppression, so a player on the rival-lab surface can click "Continue investigation to synthesis" (the path `rival-lab.spec.ts` itself drives) and advance behind the critique; likewise submit an indefensible conclusion in `review` after `revision.saved` and then complete, and the completion snapshot is taken with the player pinned on the rival lab. Self-recoverable via the revise control, but the state is wrong. Add `rivalLabCritique: undefined` to both returns.
- [x] [Review][Patch] The AC2 "critique blocks nothing" test asserts a tautology, and the challenge is already gone by its second line [tests/integration/RivalLabCritique.test.ts:351] — `expect(store.dispatch({ type: 'consultation.requested' }).ok || true).toBe(true)` is `true` for every possible value and would pass with the consultation feature deleted. Worse, the first dispatch in the test (`theory.supportRunUnselected`) routes through `withTheory`, which this same diff changed to clear `rivalLabCritique` — so the two real assertions run with no standing challenge. Assert `.ok` directly and re-establish the critique between dispatches.
- [x] [Review][Patch] Half the French punitive-language guard can never match [tests/integration/RivalLabCritique.test.ts, `punitive` regex] — `\b` in a non-unicode JS regex is defined against ASCII `\w`, so `\béchec` requires a word character before `é` and `/\béchec\b/i.test('un échec')` is `false` (verified). The most obvious French failure word in the list is unmatchable. `score` is also duplicated verbatim across both alternation groups. Use the `u` flag with explicit boundaries, or drop `\b` around the accented alternatives.
- [x] [Review][Patch] Two different refusals share `invalid-critique-timestamp`, and the ordering message is dead text [src/core/store/AppState.ts:696, src/core/store/AppState.ts:700-702] — the malformed-timestamp branch and the ordering branch return the same code, and `en.ts`/`fr.ts` define one message for it. A player refused for ordering reads "Provide a valid UTC submission timestamp." for a control that takes no timestamp input. Add a distinct code plus its EN and FR keys.
- [x] [Review][Patch] The conclusion heading's wrap was narrowed by 248px while the guide stayed at a fixed `y` [src/adapters/phaser/renderers/ColleagueRenderer.ts:49-50, src/adapters/phaser/renderers/ColleagueRenderer.ts:188-189] — `CONCLUSION_HEADING_WRAP = 944 - 232 - 16 = 696` narrows a 25px heading's bound while `HEADING_Y = 30` and `GUIDE_Y = 68` leave it 38px of vertical budget, and nothing measures the heading's height. Latent: the current EN and FR headings both fit one line at 696px. This is exactly the defect class the 1.11 and 1.12 reviews found and that `RivalLabRenderer.layout`'s own doc comment cites — reintroduced one file over in the same diff. `french-typography.spec.ts` measures per-token width, not wrapped line count, so a two-line heading passes it. Place the guide against the heading's measured height, as `cardGeometry` already does from the guide.

**Deferred**

- [x] [Review][Defer] A fourth unlinked copy of the book-control coordinate [tests/e2e/rival-lab.spec.ts:28] — deferred, pre-existing
- [x] [Review][Defer] The French e2e case asserts nothing locale-dependent [tests/e2e/rival-lab.spec.ts] — deferred, pre-existing
- [x] [Review][Defer] `french-typography.spec.ts` cannot detect a fixed-height control label wrapping to two lines [tests/e2e/french-typography.spec.ts] — deferred, pre-existing
- [x] [Review][Defer] Two vacuous unit tests, and the documented authored-order tie-break is untested [tests/unit/RivalLabRules.test.ts] — deferred, pre-existing
- [x] [Review][Defer] `critiqueHistory` grows without bound or dedupe on repeated submissions of the same proposal [src/core/store/AppState.ts:724] — deferred, pre-existing
- [x] [Review][Defer] `rivalLab.name` renders the English honorific "Mr." under FR [public/cases/young-interference/case.json] — deferred, pre-existing
- [x] [Review][Defer] `critiqueHistory` is persisted but displayed on no surface [src/ui/print/CaseRecordPrintView.ts] — deferred by decision 2026-08-06: AC2 is met by the durable fact, and a reader-facing surface belongs with the debrief/print pass since `DebriefScene` is still a placeholder

**Dismissed as noise (7)** — lexical vs `Date` timestamp comparison between reducer and validator (equivalent: `CaseRecordSchema.ts:16-19` pins canonical ISO); the reducer re-deriving the defensible set rather than calling `selectDefensibleConclusionProposalIds` (forced — `selectors.ts` imports `AppState.ts`, so a value import back would be a cycle); `RivalLabRenderer`'s degraded no-critique path drawing a headed surface (unreachable — the reducer clears `rivalLabCritique` when no critique resolves, so the router never activates the scene); `ACCENT_FALLBACK` duplicating the authored accent (overwritten by `render()` on the first frame); `SUBMIT_FILL`/`CONTROL_FILL` sharing a literal; `selectRivalLabCritique` defined twice under one name (aliased at the single call site where they meet); AC5's "identity string carries both `en` and `fr`" unmet for `rivalLab.name` (Task 1 and project-context both override — proper nouns stay plain strings).
