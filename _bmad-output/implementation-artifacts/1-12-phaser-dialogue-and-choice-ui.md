...
Code review 2026-08-06 (gds-code-review, three parallel layers: Blind Hunter, Edge Case Hunter,
Acceptance Auditor). **All three ACs verified MET.** Every finding below was independently
re-verified against the tree — including against the installed Phaser 4.2.1 **source**, not only its
typings — before being recorded. Three further findings were dismissed as noise (listed at the end).

**Verification claims reconcile exactly**, measured independently of the Dev Agent Record:
`npm run typecheck` clean · `npm test` **405 passing / 33 files** · `npm run test:e2e` **33 passed,
7 failed — precisely the seven tracked in `deferred-work.md`**. No new failures, no substituted
failures. The 1.10 review's documentation-drift defect is not present here.

**The dominant pattern in this review:** every high-severity finding is *correct for the content
authored today* and *tripped by a future copy edit the code itself permits*. The layout became
measured (as the story required) but the authored inputs it measures stayed unbounded.

#### Decisions resolved by Alexis, 2026-08-06 — each became a patch

| # | Finding | Decision |
|---|---|---|
| D1 | Card content can exceed its card at three body lines | **Lower `BODY_MAX_LINES` to 2** — match the code to the budget actually measured, so the overflow is unreachable by construction rather than guarded after the fact. |
| D2 | `ProposalChoice` value-imports from `selectors.ts` | **Move `formatAttribution` out** into a Phaser-free formatting module both `selectors.ts` and the widgets import — restores the structural barrier while keeping the fallback shared. |
| D3 | Proposal-card fallback string renders in the dialogue speaker slot | **Add a dialogue-specific key** `colleague.unattributedSpeaker` in EN+FR, used only by the dialogue surface. |

#### Decision detail (as raised)

- [x] [Review][Decision] **Card content can exceed its card once a claim wraps to three lines** — `BODY_MAX_LINES = 3` (`ProposalChoice.ts:40`) permits a 3-line claim, but the measured budget assumes two: content becomes ≈135px inside a card of 121–126px, and `limitation.setY(body.y + body.height + LIMITATION_TOP_GAP)` (`ProposalChoice.ts:145`) has **no clamp** against `this.top + this.height`. The overflow is not clipped — it paints into `CARD_GAP` and ~4px onto the **next** colleague's card background, attaching a stated limitation to the wrong claim. This fit before 1.12 (cards were 143px); shrinking them to 121–126px without lowering `BODY_MAX_LINES` opened it. `LocalizedTextSchema` is `min(1)` with no maximum, and the FR typography spec measures per-token width only, never wrapped line count — so ~50 characters of copy-editing ships it. **Options:** (a) lower `BODY_MAX_LINES` to 2 to match the measured budget; (b) clamp the limitation within card bounds and accept clipping there; (c) bound authored claim length in the schema; (d) combination. Truncation is the failure mode AC1 names, so this is a content-policy-vs-layout-policy call, not a mechanical fix.
- [x] [Review][Decision] **`ProposalChoice` imports from the selectors module, which Task 3 explicitly forbade** — `import { formatAttribution, type LocalizedProposalProjection } from '../../../core/store/selectors'` (`ProposalChoice.ts:5`) is a *value* import, so the widget's module graph now includes every selector, `selectDefensibleConclusionProposalIds` among them. The substantive guarantee holds — `formatAttribution` takes `(t, {colleagueName, roleLabel})` and no `AppState`, so the widget still cannot reach evidence or defensibility — but the spec asked for a **structural** barrier and got a convention. The Completion Notes disclose the extraction as a benefit without noting it breaks the stated rule. **Options:** (a) move `formatAttribution` out of `selectors.ts` into a Phaser-free formatting module, restoring the structural barrier; (b) amend the rule to "no selector that takes `AppState`" and record why. Note the type import of `LocalizedProposalProjection` was prescribed by the story's own widget shape, so *some* module edge was unavoidable.
- [x] [Review][Decision] **"Unattributed proposal" now renders in the dialogue speaker slot** — sharing `formatAttribution` between the proposal cards and the dialogue speaker line was right for the `—`-with-empty-role defect, but the standalone fallback string was not re-checked for a surface that is not a proposal card. On a degraded `case.json` whose beat `speakerId` is outside `colleagues[]`, the dialogue panel renders **"Unattributed proposal"** above a line of spoken prose; the new unit test pins that string verbatim (`DialogueBeats.test.ts`). **Options:** (a) add a distinct `colleague.unattributedSpeaker` key (EN+FR) for the dialogue surface; (b) reword the shared label to read correctly on both; (c) accept it as a degraded-content-only path. Any new key inherits the EN+FR requirement.

#### Patches

_The first three carry out decisions D1–D3 above._

- [x] [Review][Patch] D1 — lower `BODY_MAX_LINES` from 3 to 2 so a claim cannot exceed the measured card budget [`src/adapters/phaser/ui/ProposalChoice.ts:40`]
- [x] [Review][Patch] D2 — extract `formatAttribution` into a Phaser-free formatting module and drop the `selectors.ts` value import from the widget [`src/adapters/phaser/ui/ProposalChoice.ts:5`, `src/core/store/selectors.ts:209-212`]
- [x] [Review][Patch] D3 — add `colleague.unattributedSpeaker` (EN+FR) for the dialogue speaker slot and use it in `selectDialogueBeats`; update the pinned assertion [`src/core/i18n/locales/en.ts`, `fr.ts`, `src/core/store/selectors.ts`, `tests/unit/DialogueBeats.test.ts`]
- [x] [Review][Patch] Beat-index signature is neither phase-qualified nor injective, so a phase change can resume mid-conversation [`src/adapters/phaser/ui/DialogueBox.ts:158`, `:240-251`] — the "same conversation" key is `beats.map(({id}) => id).join(' ')`. `SceneRouter.ts:58` (`if (nextSceneKey === activeSceneKey) return;`) does not restart a scene when the sceneKey is unchanged, and `TheoryBoard` hosts **both** `synthesis` and `review`, so one `DialogueBox` instance sees both conversations. The schema at `CaseDefinitionSchema.ts:508-511` **deliberately permits** a beat id to repeat across scenes. Two same-length conversations sharing ids would make `review` open on its **last** beat, already labelled `dialogue.end`, with earlier beats unreachable (`advance()` only moves forward, and the `Math.min` clamp lands on the final beat). Second, independent break: `stableId` is `z.string().trim().min(1)` — spaces are legal — so `['a b']` and `['a','b']` produce the identical signature, directly contradicting the comment at `:157` ("joined by a space that cannot occur inside a stable id"). Latent only because today's three id lists happen to be disjoint. Fix: key on the scenario-scene identity (phase or scene index), not the id list.
- [x] [Review][Patch] The `setBounds` hit-area re-apply is inert — the resize survives only by accident [`src/adapters/phaser/ui/ProposalChoice.ts:122-132`] — verified in Phaser 4.2.1 source: `InputPlugin.enable` (`src/input/InputPlugin.js:934`) is `if (gameObject.input) { gameObject.input.enabled = true; }`, so a second `setInteractive()` **never** rebuilds the hit area, and `disable` leaves `input` in place. `applyInputState()` therefore cannot do what the comment at `:127-130` claims. What actually keeps the hit area correct is `Rectangle.setSize` (`src/gameobjects/shape/rectangle/Rectangle.js:146`), which updates `input.hitArea` — but **only `if (input && !input.customHitArea)`**. The moment anyone supplies an explicit hit area (the documented way to inset a click target), `customHitArea` flips true, `setSize` stops touching it, and every card is silently clickable in a band it no longer occupies — the exact defect this code was written to prevent. Behaviour is correct today; the reasoning credited with making it correct is a no-op. Fix: resize `input.hitArea` explicitly and correct the comment.
- [x] [Review][Patch] The dialogue panel grows without bound; `getBottomY()` is never clamped, so cards can land off-canvas [`src/adapters/phaser/ui/DialogueBox.ts:200-205`, `src/adapters/phaser/renderers/ColleagueRenderer.ts:206-210`] — `available = 768 - top - 16` can go negative. `Math.max(MIN_CARD_HEIGHT, …)` rescues `height` but **nothing rescues `top`**. Past ~18 wrapped body lines the third and fourth cards begin below y=768 on a fixed 1024×768 `Scale.FIT` canvas with no scroll: not visible, not clickable, so the phase cannot be completed from the canvas at all. This is what the `MIN_CARD_HEIGHT = 72` floor engaging actually produces — it prevents a negative height, not an off-canvas layout. Fix: clamp so four cards always remain on canvas.
- [x] [Review][Patch] `dialogueBeats: []` fails the base parse and so silences **every** authored-content refinement [`src/schemas/CaseDefinitionSchema.ts:265`] — `z.array(ScenarioDialogueBeatSchema).min(1).optional()`. Zod skips `superRefine` once the base parse fails, so an author writing the natural `"dialogueBeats": []` gets a generic `too_small` and loses every cross-field message at once: unresolved `speakerId`, `encodesPath`, duplicate beat ids, proposal attribution, the defensible-conclusion rule. `ScenarioScriptSchema` documents avoiding exactly this by omitting `.min` on `scenes`; the new field reintroduces it one level down. Fix: drop `.min(1)` and let the absent-or-empty forms both mean "no conversation".
- [x] [Review][Patch] No English dialogue copy is measured, and an unbreakable token overflows the panel horizontally [`tests/e2e/french-typography.spec.ts`, `src/adapters/phaser/ui/DialogueBox.ts:130-132`] — the whole typography suite runs `locale: 'fr-FR'` and samples `text.fr`, so *any* over-wide English token is unmeasured. Phaser word-wrap cannot break inside a word, so a URL or hyphen-free compound wider than the 908px body bound draws past the panel's right edge (panel spans x 40→984) and past the 1024px canvas beyond ~926px. The panel grows only vertically, so it never contains the overflow.
- [x] [Review][Patch] The completion state machine is unobservable, and a one-beat conversation gives the advance control no response [`src/adapters/phaser/ui/DialogueBox.ts:219-221`, `:253-270`] — `isLast` is true on the *first* paint of a single-beat scene, so the control is labelled `dialogue.end` and filled `CONTROL_FILL_ENDED` before it is ever clicked; clicking then takes the completion branch and calls `paint()`, which recomputes identical text, fill, position and height. The frame is pixel-identical: a live hand-cursor target with no feedback, and that `paint()` call is dead. `isComplete()` has **zero callers** anywhere in `src`/`tests`, and `onComplete` is declared and fired but supplied by no consumer, while `destroy()` dutifully resets a field nothing reads. `dialogue-advance.spec.ts` passes only because it clicks a non-final beat of a 3-beat conversation. Fix: give `completed` a distinct rendering, or remove the dead `isComplete`/`onComplete` surface.
- [x] [Review][Patch] The `scene-router.spec.ts` probe was re-pointed at an assertion with no failing mode [`tests/e2e/scene-router.spec.ts`] — `not.toHaveValue(TYPED_CONCLUSION)` became `expect([TYPED_CONCLUSION, ...AUTHORED_CLAIMS]).toContain(await conclusionField.inputValue())`. If the click hits **no card** — precisely the regression 1.12 makes reachable by moving the card band and rederiving hit areas — the field still holds `TYPED_CONCLUSION`, the first element of the expected array, and the test passes. Compounding it, the non-retrying `inputValue()` is justified by "settled by the awaited assertion above", but that assertion is `toHaveValue(slitSpacingBeforeClick)` — an assertion that a value is *unchanged*, which satisfies on its first poll and waits for nothing, so the read can legitimately return the pre-click value. Both races resolve toward a pass. The Debug Log's reasoning for not pinning "a card was hit" is sound (it would pin a 2px coincidence AC1 requires to move) — but the replacement must still be able to fail.
- [x] [Review][Patch] Two "beat position is never persisted" guards assert a naming convention, not a behaviour [`tests/integration/DialogueAndChoice.test.ts:193`, `:202`] — both reduce to `expect(Object.keys(x).filter((key) => /dialogue|beat/i.test(key))).toEqual([])`. Neither test reads, advances, or completes a conversation, and the regex covers only *top-level* key names. A persisted beat index named `scenePosition`, `conversationIndex`, `currentLine`, or nested under an existing key passes both unchanged. These are the stated guard for the ephemeral-beat decision and cannot detect it being violated.
- [x] [Review][Patch] Three test sweeps go vacuous together on empty input [`tests/integration/DialogueAndChoice.test.ts`] — `reads differently in French` (`toHaveLength(english.length)` then `toEqual([])` is `0===0` and `[]===[]` when both are empty), `projects nothing beyond what the widget renders` (a bare `for…of` — zero assertions on an empty list), and `keeps beat IDs unique within each conversation` (same shape). A `case.json` edit dropping `dialogueBeats`, or a `selectDialogueBeats` regression returning the frozen empty array unconditionally, silently empties all three while they report green. The sibling test and the new typography sweep both carry `expect(beats.length).toBeGreaterThan(0)` — the pattern is known and was applied to two of five sweeps over the same content.
- [x] [Review][Patch] `DIALOGUE_TOP = 118` is an unmeasured literal against the guide line above it [`src/adapters/phaser/renderers/ColleagueRenderer.ts:49-51`] — the relationship the story demanded be measured (cards from `getBottomY()` + `CARDS_GAP`) is correctly measured and `CARDS_TOP = 132` is genuinely gone, but the guide→panel relationship one layer up is the identical defect class the 1.11 review named ("two objects sharing a fixed window with ~3px of slack and no measurement"). `GUIDE_Y = 68` with a wrapped 15px two-line French guide ends near y≈107, leaving ~11px of unmeasured slack — and that same slot also renders the transient error, so error copy shares the budget. A three-line guide or error would be overdrawn by the panel. Not reachable with today's copy.
- [x] [Review][Patch] `dialogue-advance.spec.ts` clicks a fixed `y = 500` under a comment asserting the layout guarantees a hit [`tests/e2e/dialogue-advance.spec.ts`] — the comment claims the coordinate "is inside a card whatever the panel's measured height: the cards divide everything below it." They do not divide it continuously: there is a 10px `CARD_GAP` between each, and the band's top moves with the panel. One extra wrapped beat line shifts the boundaries so `y = 500` falls in a gap, the click records nothing, and the test fails as `expect(prediction).not.toHaveValue('')` with no hint the cause was a copy edit. This is the same coordinate-coincidence class the diff correctly *removes* from `scene-router.spec.ts`, reintroduced in the new spec. Fix: derive the click point from the widget's exported constants.
- [x] [Review][Patch] AC1's advance behaviour rests on a screenshot inequality inside `toPass()` [`tests/e2e/dialogue-advance.spec.ts:57-60`] — `expect(Buffer.compare(await canvas(page).screenshot(), firstBeat)).not.toBe(0)` retries until *any* pixel differs, so any canvas nondeterminism would satisfy it independently of the repaint under test. It does discriminate today (the surface is static and the dev mutation-checked it by reverting the fix), but it is weaker than it reads.
- [x] [Review][Patch] Three constants were exported to satisfy the shared-constant rule, then not read by the spec that restates their literals [`src/adapters/phaser/ui/ProposalChoice.ts:48-55`, `tests/e2e/french-typography.spec.ts:77-81`] — `PROPOSAL_ATTRIBUTION_FONT_SIZE`, `PROPOSAL_MARKER_FONT_SIZE`, and `PROPOSAL_MARKER_WRAP` are exported while the spec still hard-codes `fontSize: 15` for `colleague.attribution`/`colleague.unattributed` and `fontSize: 15, wrapWidth: 160` for `proposal.selected`/`proposal.choose`. `PROPOSAL_MARKER_WRAP` is a dead export. The literals are pre-existing from 1.11, so 1.12 adds no new unlinked pair — but it shipped the fix and left it unapplied on the four rows it covers.
- [x] [Review][Patch] Stale cross-reference in a new test's docstring [`tests/unit/DialogueBeats.test.ts:12`] — "That the *authored* beats resolve is asserted in `tests/integration/ProposalSelection.test.ts`, against `case.json`." That file contains no beat or `selectDialogueBeats` reference; the assertions are in `tests/integration/DialogueAndChoice.test.ts`, created by this same change. Anyone auditing the split-coverage claim is sent to the wrong file.

#### Patch outcomes — all 17 applied 2026-08-06

**Measured after the patches, not asserted:** `npm run typecheck` clean · `npm test` **424 passing,
34 files** (from 405 / 33 — the new file is `tests/unit/DialogueBox.test.ts`) · `npm run test:e2e`
**33 passed, 7 failed — the same seven tracked in `deferred-work.md`**, no new failures.

Where the fix differed from the finding's suggestion, or turned up something else:

| Patch | What was done, and anything the fix revealed |
|---|---|
| Beat conversation key | `render` now takes an explicit `conversationId`, supplied by the owner as the **live phase**. Deriving it inside the widget was the defect, so the widget no longer derives anything: `DialogueBeatView.id` is documented as carried for structural match and diagnostics, not as the key. **Mutation-checked** — reverting to the id-joined key fails `restarts the conversation when the phase changes even though the beat ids are identical`. |
| Hit-area resize | The first attempt used `instanceof Phaser.Geom.Rectangle`, which needs a *value* import of Phaser — and Phaser touches `window` at import time, so it broke the entire Playwright suite at collection (this module is imported by the specs for its constants). Rewritten to write `hitArea.width/height` directly, which is what `Geom.Rectangle.setSize` does anyway. Worth recording: the fix for a Phaser-semantics bug was itself a Phaser-semantics bug. |
| Panel growth clamp | `cardGeometry` now clamps the measured top so four cards always fit at `MIN_CARD_HEIGHT`. Chosen deliberately over bounding the beat text: overlap is a legible fault an author can see, whereas cards leaving the canvas is invisible and makes the phase uncompletable. |
| `DIALOGUE_TOP` measured | Implemented as a **floor**, `max(DIALOGUE_TOP, guideBottom + DIALOGUE_GAP)`, not a replacement. Today's guide (one EN line, two FR) measures at or below the constant, so the layout does not move and the click target the e2e specs derive stays valid — the measurement only adds the case the constant could not survive. Required a new `DialogueBox.setTop`. |
| Completion observability | `completed` now drives the control fill, the label alpha, **and** the hand cursor, so the click that finishes a one-beat conversation changes something. `dialogue.end` still appears on arrival at the last beat, which is what Task 2 asks for. **Mutation-checked** — reverting the fill to `isLast` fails `responds observably to the click that completes a single-beat conversation`. `isComplete()`/`onComplete` were kept: they are the story's prescribed widget shape and now carry real meaning, though still no consumer needs them. |
| Speaker fallback (D3) | New `colleague.unattributedSpeaker` in EN+FR, and `projectAttribution` now takes the unresolved-label key as a **required** parameter — a default would have turned choosing correctly into remembering to, which is the shape the i18n rules already forbid for `locale`. Added to the typography sweep. |
| `formatAttribution` extraction (D2) | Moved to `src/core/i18n/formatAttribution.ts`. `ProposalChoice`'s remaining import of `LocalizedProposalProjection` is now `import type`, so it is erased and the selectors module is genuinely out of the widget's runtime graph rather than merely unused by it. |
| Persistence guards | Replaced the `/dialogue|beat/` name filters with **exact key sets** for both the state and the record, so any field arriving fails whatever it is called; added a test that resolves every beat of every phase twice and asserts the state is unchanged **by reference**. |
| Vacuous sweeps | Non-empty guards on all three, plus one replacement test asserting the content fact that made the conversation-key bug reachable (two distinct conversations behind one `TheoryBoard` scene key). A first draft of that test ended in `expect('synthesis').not.toBe('review')` — a tautology, removed. |
| `dialogue-advance` probe | Fixed `y = 500` replaced by an exported `lastProposalCardProbe(canvasHeight)`, anchored to the canvas floor so it stays inside the last card wherever the band starts. The screenshot assertion is now **clipped to the dialogue panel's band** and preceded by a stability check proving an unchanged panel compares equal — so canvas noise elsewhere can no longer satisfy it. |
| `scene-router` probe | The scene-teardown assertion was already sound and can fail; it was left alone. The bolted-on "the field holds one of these" check was replaced with a **consistency invariant** — conclusion and limitation must either both be the player's words or both come from the same authored proposal — which rejects a partial or blended write. Card hit-testing was moved out of this spec's remit and left to `dialogue-advance.spec.ts`, which now probes a derived coordinate. |
| Schema `.min(1)` | Removed, with two new tests: an empty list parses, and an empty list elsewhere in the script no longer suppresses a real cross-field message. |
| English typography | The authored beats and all proposal copy are now measured in **both** locales. Width measurement is locale-independent, so this needed no second spec run. Also fixed the four rows that restated `15`/`160` instead of reading the constants the story exported, and `PROPOSAL_MARKER_WRAP` is no longer a dead export. |

**Not fixed, and why:** `isComplete()` and `onComplete` still have no consumer. They are the widget shape
the story specifies and are now behaviourally meaningful; deleting them would be a spec deviation to
remove two lines. Flagged rather than silently kept.

#### Dismissed as noise (recorded for traceability)

- **`dialogue.end` shown on arrival at the last beat, not after it** — flagged by the Blind Hunter as an off-by-one. It is what Task 2 asks for verbatim: "the last beat's control reads 'done'-style copy and further clicks are no-ops." Working as specified; the blind layer had no spec by design.
- **`layoutAndRenderCards` skips `setBounds` for a card whose projection is missing** — unreachable. `kind` is fixed at construction, `project(state)` reads the immutable `state.caseDefinition[kind]Proposals` that created the cards, and `createStore.ts:54` passes `state.caseDefinition` straight back on import, so the definition is never swapped in-session. The guard is defensive-only.
- **Counter (13px) and advance-control (14px) type sit below the 16px body floor** — ≈12.2 and ≈13.1 CSS px after the 0.9375 factor. The story's floor is explicitly scoped to "dialogue **body** text", the body is correctly 16, and ADR-008 de-scopes a11y from MVP. Within the letter of the spec.
...
- `src/core/i18n/formatAttribution.ts` — added by the code review (D2)
- `tests/unit/DialogueBox.test.ts` — added by the code review; the widget had no test at all
...
| 2026-08-06 | 1.3     | Code review (three parallel layers): all three ACs verified MET and every verification claim reconciled independently. 3 decisions resolved and 17 patches applied — `BODY_MAX_LINES` 3→2, an explicit phase-keyed `conversationId`, a hit-area resize that actually resizes, a card-band clamp, `formatAttribution` extracted out of `selectors.ts`, a dialogue-specific speaker fallback (EN+FR), observable completion state, the schema `.min(1)` removed, English typography measured, and seven tests that could not fail made able to fail. New `tests/unit/DialogueBox.test.ts`. 424 passing / 34 files; e2e unchanged at 33 passed / the same 7 baseline failures | Alexis |
